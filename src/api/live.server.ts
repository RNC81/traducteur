...
export const addTranscriptServer = async (data: {
  share_code: string;
  ...
  if (!user) throw new Error("Unauthorized: vous devez être connecté pour diffuser.");
  ...
  if (session.owner.toString() !== user.id) {
    throw new Error("Interdit : vous n'êtes pas propriétaire de cette session.");
  }
